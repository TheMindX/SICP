...
(define (get_bits char tree)
  (define (get_bits_help subtree subbits)
    (if (leaf? subtree)
        (if (eq? (symbol-leaf subtree) char)
            subbits
            null)
        (let ((leftbits (get_bits_help (left-branch subtree) (append subbits '(0))  )))
          (if (null? leftbits)
              (let ((rightbits (get_bits_help (right-branch subtree) (append subbits '(1) ))))
                (if (null? rightbits)
                    null
                    rightbits))
              leftbits))))
